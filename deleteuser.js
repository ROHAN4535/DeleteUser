...
window.addEventListener("DOMContentLoaded", ()=> {
    axios.get("https://crudcrud.com/api/ac8acfd1ee5448d5873743091a9610e7/appointmentData")
        .then((response) => {
            console.log(response)
...
function showUserOnScreen(userData){
   ...
   const childHTML =`<li id=${userData._id}> ${userData.name} - ${userData.email}
                       ...
                       <button onclick=editUserDetails('${userData._id}','${userData.name}')> Edit</button>                 
                   </li>`
   ...
   function deleteUser(userId){
       axios.delete(`https://crudcrud.com/api/ac8acfd1ee5448d5873743091a9610e7/appointmentData/${userId}`)
           .then((response) => {
